...
(** Internal function: finds the representant identifier of a given identifier.
 * It may raise [Not_found] if [i] is not present in the mapping. **)
let rec representant p i =
  let pi = PMap.find i !p in
  if i = pi then pi
  else
    let pi' = representant p pi in
    Utils.assert_ __LOC__ (let pi'' = PMap.find pi' !p in pi' = pi'') ;
...
let same_class mp e1 e2 =
  Utils.if_option (find mp e1) (fun i1 ->
    Utils.if_option (find mp e2) (fun i2 ->
      Some (i1 = i2)))

let merge_idt mp e1 e2 =
  let (i1, mp) = find_insert mp e1 in
  let (i2, (m, p)) = find_insert mp e2 in
  Utils.assert_ __LOC__ (let pi2 = PMap.find i2 !p in i2 = pi2) ;
  Utils.assert_ __LOC__ (let pi1 = PMap.find i1 !p in i1 = pi1) ;
  (i2, (m, ref (PMap.add i1 i2 !p)))

let merge mp e1 e2 =
  let mp = snd (merge_idt mp e1 e2) in
  Utils.assert_ __LOC__ (same_class mp e1 e2 = Some true) ;
  mp
...
let to_list (m, p) =
  ...
  List.map (Utils.assert_option __LOC__) representants
...
let get_one_class (m, p) =
  let r = ref None in
  try
    PMap.iter (fun _i ip -> r := Some ip ; raise Found) !p ;
    None
  with Found ->
    let i = Utils.assert_option __LOC__ !r in
    Some (Utils.assert_option __LOC__ (Id.map_inverse m (representant p i)))

let one_class (m, p) =
  match get_one_class (m, p) with
  | None -> true
  | Some e ->
    let i = Utils.assert_option __LOC__ (Id.get_id m e) in
    try
